...
namespace CommunityCar.Application.Services.Authorization;
...
public class PermissionService : IPermissionService
{
    ...
    public async Task<IEnumerable<PermissionDTO>> GetUserPermissionDetailsAsync(Guid userId)
    {
        ...
        var allPermissions = userPermissions.Select(up => new PermissionDTO
        {
            Id = up.Permission.Id,
    ...
    public async Task<IEnumerable<PermissionDTO>> GetRolePermissionDetailsAsync(string roleName)
    {
        var role = await _roleRepository.GetByNameAsync(roleName);
        if (role == null) return Enumerable.Empty<PermissionDTO>();

        var rolePermissions = await _permissionRepository.GetRolePermissionsAsync(role.Id);
        return rolePermissions.Select(rp => _mapper.Map<PermissionDTO>(rp.Permission));
    }
    ...
    public async Task<IEnumerable<UserPermissionSummaryDTO>> GetUsersWithPermissionAsync(string permission)
    {
        ...
        return Enumerable.Empty<UserPermissionSummaryDTO>();
    }

    public async Task<IEnumerable<RolePermissionSummaryDTO>> GetRolesWithPermissionAsync(string permission)
    {
        ...
        return Enumerable.Empty<RolePermissionSummaryDTO>();
    }
    ...
    public async Task<IEnumerable<PermissionAuditDTO>> GetUserPermissionAuditAsync(Guid userId, int page = 1, int pageSize = 50)
    {
        // This would need to be implemented with an audit repository
        return Enumerable.Empty<PermissionAuditDTO>();
    }

    public async Task<IEnumerable<PermissionAuditDTO>> GetRolePermissionAuditAsync(string roleName, int page = 1, int pageSize = 50)
    {
        // This would need to be implemented with an audit repository
        return Enumerable.Empty<PermissionAuditDTO>();
    }

    public async Task<IEnumerable<PermissionAuditDTO>> GetPermissionAuditAsync(string permission, int page = 1, int pageSize = 50)
    {
        // This would need to be implemented with an audit repository
        return Enumerable.Empty<PermissionAuditDTO>();
    }
    ...
    private async Task<IEnumerable<PermissionDTO>> GetUserRolePermissionsAsync(Guid userId)
    {
        var userRoles = await _roleRepository.GetUserRolesAsync(userId);
        var allRolePermissions = new List<PermissionDTO>();
