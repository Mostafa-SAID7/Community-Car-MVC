namespace CommunityCar.Application.Features.Chat.ViewModels;

/// <summary>
/// Chat ViewModels
/// 
/// This file has been reorganized for better maintainability. Each ViewModel class
/// is now in its own separate file within this namespace. This improves code organization,
/// reduces merge conflicts, and makes the codebase easier to navigate.
/// 
/// Individual files:
/// - ConversationVM.cs - Conversation with participants and last message
/// - MessageVM.cs - Individual chat message with sender info
/// - ParticipantVM.cs - Chat participant with online status
/// - UserOnlineStatusVM.cs - User online status indicator
/// - TypingIndicatorVM.cs - Typing indicator for real-time chat
/// - CreateConversationVM.cs - Request to create new conversation
/// - SendMessageVM.cs - Request to send a message
/// 
/// All classes maintain their original functionality and public API.
/// </summary>

// The individual classes are now in separate files within this namespace.
// This file is kept for documentation and backward compatibility.